...
; Ver 0.2     13 August 2015  Charley Picker <charleypicker@yahoo.com>
;                             Converted to Openwatcom
;                             Modified code to be relocatable
;**********************************************************************
...
; Select 38400 baud rate from table above
BRATE_LOW   EQU   0x03
BRATE_HIGH  EQU   0x00
...
;------------------------------------------------------------------------------------
BASE_SEGMENT    EQU  0400h                      ; First 1024 bytes used for int vectors

;------------------------------------------------------------------------------------
; Default MON88 Code segment size
; Note: Code segment must be writeable, as it also includes writeable stack
;------------------------------------------------------------------------------------
MON88_SIZE           EQU 0x4000h                 ; By default we are 16KBytes in size

;------------------------------------------------------------------------------------
; Diagnostic postcode port
;------------------------------------------------------------------------------------
IO_POST              EQU 080h
...
INITMON:    POSTCODE 0x88                       ; Magic number for MON88
...
; Using standard values from baudrate divisor table found above
;----------------------------------------------------------------------
...
;----------------------------------------------------------------------
            ...
            MOV     CX, CS                      ; Exactly what segment are we located?
NEXTINTS:   MOV     WORD PTR ES:[BX], OFFSET INTX   ; Spurious Interrupt Handler offset
            MOV     WORD PTR ES:[BX+2], CX      ; Point to our segment!!
            ADD     BX,4
            CMP     BX,0400h
            JNE     NEXTINTS      
                         
            MOV     WORD PTR ES:[0004], OFFSET INT1_3 ; INT1 Single Step handler
            MOV     WORD PTR ES:[0012], OFFSET INT1_3 ; INT3 Breakpoint handler
            MOV     WORD PTR ES:[0064], OFFSET INT10  ; INT10h  
            MOV     WORD PTR ES:[0088], OFFSET INT16  ; INT16h  
            MOV     WORD PTR ES:[0104], OFFSET INT1A  ; INT1A, Timer functions  
            MOV     WORD PTR ES:[0132], OFFSET INT21  ; INT21h  
...
SETBREAKP:  MOV     BX,OFFSET BPTAB             ; BX point to Breakpoint table
            ...
            MOV     BYTE PTR [BX+3],1           ; Enable Breakpoint
            WRSPACE
...
CLRBREAKP:  MOV     BX,OFFSET BPTAB             ; BX point to Breakpoint table
            ...
            SHL     AL,1                        
            ADD     BX,AX                       ; point to table entry 
            MOV     BYTE PTR [BX+3],0           ; Clear Breakpoint
...
NEXTCBP:    MOV     AX,8
            ...
            TEST    BYTE PTR [BX+3],1           ; Check enable/disable flag
            JZ      NEXTDBP
...
NEXTENBP:   MOV     AX,8
            ...
            MOV     BYTE PTR ES:[DI],0CCh       ; Write INT3 instruction to address
...
TRACNENTRY: MOV     AX,[UAX]                    ; Restore User Registers
            ...
            PUSH    WORD PTR [UFL]
            PUSH    WORD PTR [UCS]              ; Push CS (Base Segment) 
            PUSH    WORD PTR [UIP]       
            MOV     DS,[UDS]
            IRET                                ; Execute!
...
WAITTX:     IN      AL,DX                       ; read status
            ...
            JZ      WAITTX                      ; no, wait
            MOV     DX,DATAREG                  ; point to data port
            POP     AX
            OUT     DX,AL
            POP     DX
            RET
...
NEXTUREG:   MOV     AX,SS:[DI]                  ; Get register
            ...
            MOV     [UFL],AX
            AND     WORD PTR [UFL],0FEFFh                ; Clear TF
...
CONTBPC:    DEC     WORD PTR [UIP]              ; No, IP-1 and save  
...
INT3RES:    MOV     AL,BYTE PTR [BX+2]          ; Get original Opcode
...
ISR16_01:   CMP     AH,01h
            ...
            OR      WORD PTR SS:[BP+8],0040h    ; SET ZF in stack stored flag
            ...
            MOV     DX,DATAREG    
            IN      AL,DX                       ; return result in al
            AND     WORD PTR SS:[BP+8],0FFBFh   ; Clear ZF in stack stored flag
...
ISR1A_RET:  AND     WORD PTR SS:[BP+8],0FFFEh   ; Clear Carry to indicate no error
            POP     BP
            POP     DS      
            IRET        
...
ISR21_0B:   CMP     AH,0Bh
            ...
            XOR     AH,AH
            MOV     DX,STATUS                   ; get UART RX status
            IN      AL,DX
            AND     AL,RX_AVAIL
...
;----------------------------------------------------------------------
;;_CONST      SEGMENT BYTE PUBLIC USE16 'DATA'
L$113:
    DB  0
L$114:
    DB  41H, 41H, 41H, 0
...
;;_CONST      ENDS
;;_CONST2      SEGMENT WORD PUBLIC USE16 'DATA'
;;_CONST2      ENDS
...
;----------------------------------------------------------------------
; Make this an optional ram image for cold booting 8086 in real mode
; Note: boot ram must be placed address 0xfc000 (three zero's) of
;       boot segment for boot ram option to work!!! 
;----------------------------------------------------------------------
            org   (MON88_SIZE - 0x10h)
BOOTOFFSET  EQU   0x0000
BOOTSEGMENT EQU   0xfc00
            db    0eah                          ;; Jump to absolute address opcode
            dw    BOOTOFFSET, BOOTSEGMENT       ;; Jump address

;----------------------------------------------------------------------
; End of File marker
;----------------------------------------------------------------------
            org     (MON88_SIZE - 0x8h)
EOF_MARKER  EQU     "MON88EOF"
            db      EOF_MARKER                  ;; end of file marker

_TEXT                   ends                    ;; End of code segment
                        end             mon88   ;; End of this program
