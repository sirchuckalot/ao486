...
wire cond_12 = (wb_ack_i) && (wb_cti_o == 3'b000);
wire cond_13 = (wb_ack_i) && counter == 2'd1;
wire cond_14 = (wb_ack_i) && counter != 2'd0;
wire cond_15 = state == STATE_READ;
wire cond_16 = wb_readdatavalid_i;
wire cond_17 = current_burstcount - { 1'b0, counter } == 3'd1;
wire cond_18 = current_burstcount - { 1'b0, counter } == 3'd2;
wire cond_19 = current_burstcount - { 1'b0, counter } == 3'd3;
wire cond_20 = current_burstcount - { 1'b0, counter } == 3'd4;
wire cond_21 = counter == 2'd0;
wire cond_22 = current_burstcount == 3'd4;
wire cond_23 = wb_ack_i == `FALSE;
wire cond_24 = state == STATE_READ_CODE;
wire cond_25 = counter == 2'd3;
wire cond_26 = counter == 2'd2;
wire cond_27 = counter == 2'd1;
wire cond_28 = counter < 2'd3;
//======================================================== saves
wire  wb_we_o_to_reg =
    (cond_0 && cond_4)? (          `TRUE) :
...
wire  wb_read_o_to_reg =
    ...
    (cond_15 && cond_16 && cond_21)? ( `FALSE) :
    (cond_15 && cond_23)? ( `FALSE) :
    (cond_24 && cond_16 && cond_21)? ( `FALSE) :
...
wire [1:0] counter_to_reg =
    ...
    (cond_11 && cond_14)? ( counter - 2'd1) :
    (cond_15 && cond_16)? ( counter - 2'd1) :
    (cond_24 && cond_16)? ( counter - 2'd1) :
    counter;
wire [31:0] bus_3_to_reg =
    (cond_15 && cond_16 && cond_20)? ( wb_dat_i) :
    (cond_24 && cond_16 && cond_21)? ( wb_dat_i) :
    bus_3;
wire  read_line_done_trigger_to_reg =
    (cond_0 && cond_2)? ( `FALSE) :
    (cond_15 && cond_16 && cond_21 && cond_22)? ( `TRUE) :
    read_line_done_trigger;
wire [31:0] bus_code_partial_to_reg =
    (cond_24 && cond_16)? ( wb_dat_i) :
    bus_code_partial;
wire [3:0] wb_sel_o_to_reg =
    ...
    (cond_11 && cond_14)? ( byteenable_next) :
    wb_sel_o;
...
wire [1:0] state_to_reg =
    ...
    (cond_15 && cond_16 && cond_21)? ( STATE_IDLE) :
    (cond_24 && cond_16 && cond_21)? ( STATE_IDLE) :
    state;
